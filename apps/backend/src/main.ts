...
async function bootstrap() {
    ...
    helmet({
      contentSecurityPolicy: {
        directives: {
          ...
          // Restrict where JS can make network requests — REST API only, no WebSocket URLs
          connectSrc: ["'self'"],
  ...
  if (config.isDevelopment) {
    const swaggerConfig = new DocumentBuilder()
      .setTitle("Netrek Backend API")
      .setDescription(
        "Netrek Web backend API — auth, accounts, stats, server registry",
      )
      .setVersion("0.1.0")
  ...
  console.log(
    `Netrek Backend API running on http://localhost:${config.api.port}`,
  );
  if (config.isDevelopment) {
