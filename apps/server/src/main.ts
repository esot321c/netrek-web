...
async function bootstrap() {
    ...
    helmet({
      contentSecurityPolicy: {
        directives: {
          ...
          connectSrc: ["'self'", "ws://localhost:3012", "wss://localhost:3012"],
  ...
  if (config.isDevelopment) {
    const swaggerConfig = new DocumentBuilder()
      .setTitle("Netrek API")
      .setDescription("Netrek Web game server API")
      .setVersion("0.1.0")
  ...
  console.log(`Netrek API running on http://localhost:${config.api.port}`);
  if (config.isDevelopment) {
